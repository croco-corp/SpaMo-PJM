#!/bin/bash
# Combined queue (eval first, then training):
#   1. Upstream SpaMo PHOENIX→PJM-MS eval (correct ckpt = user reproduction, BLEU 5.36 dev)
#   2. Uni-Sign PJM SI from scratch (train + eval + post-hoc BLEURT-20)
#   3. v32 SpaMo dual SI scratch (train + test eval)
#   4. v33 Mod-SpaMo quad-xattn SI scratch (train + test eval)
# Run: nohup bash scripts/queue_eval_phoenix_then_table_remaining.sh > queue_phoenix_table_$(date +%Y%m%d_%H%M%S).log 2>&1 &
set -o pipefail
cd "$(dirname "$0")/.."

LOG=queue_phoenix_table_$(date +%Y%m%d_%H%M%S).log
exec > >(tee -a "$LOG") 2>&1

UNISIGN_DIR=/home/croco/Uni-Sign
UPSTREAM_PJM_MS_CKPT=/home/croco/SpaMo/logs/2026-05-02T08-39-04_finetune_pjm_ms_upstream_v2/checkpoints/epoch=00043-step=0129844-bleu4=5.36.ckpt

source .venv/bin/activate

###############################################################################
# 1. Upstream SpaMo PHOENIX→PJM-MS eval
###############################################################################
echo "=== [1] Upstream SpaMo PHOENIX→PJM-MS eval: $(date) ==="
echo "    ckpt=$UPSTREAM_PJM_MS_CKPT"
if [ ! -f "$UPSTREAM_PJM_MS_CKPT" ]; then
    echo "ABORT — ckpt not found"
    exit 1
fi
python main.py \
    -c configs/finetune_v26_dual_paper_faithful_from_upstream.yaml \
    --ckpt "$UPSTREAM_PJM_MS_CKPT" \
    -t False \
    --test True \
    -n test_upstream_pjm_ms_dual \
    --tags test pjm-ms dual phoenix-upstream-pjm || echo "(upstream eval failed, continuing)"

deactivate

###############################################################################
# Wait for GPU idle between stages
###############################################################################
wait_gpu_idle() {
...
###############################################################################
# 2. Uni-Sign PJM SI from scratch
###############################################################################
...
if [ -f "$US_OUT_TRAIN/best_checkpoint.pth" ]; then
    ...
    echo "=== [2a] Uni-Sign SI scratch train: $(date) ==="
...
if [ -f "$US_OUT_TRAIN/best_checkpoint.pth" ] && [ ! -f "$US_OUT_EVAL/test_tmp_refs.txt" ]; then
    echo "=== [2b] Uni-Sign SI scratch eval: $(date) ==="
    mkdir -p "$US_OUT_EVAL"
    PORT=29501
...
if [ -f "$US_OUT_EVAL/test_tmp_refs.txt" ] && [ ! -f "$US_OUT_EVAL/bleurt_BLEURT-20.json" ]; then
    echo "=== [2c] BLEURT-20 post-hoc Uni-Sign SI scratch: $(date) ==="
    source .venv/bin/activate
...
###############################################################################
# 3. SpaMo v32 dual SI scratch
###############################################################################
echo "=== [3a] v32 dual SI scratch train: $(date) ==="
source .venv/bin/activate
python main.py \
...
if [ -n "$V32_CKPT" ]; then
    echo "=== [3b] v32 test eval: $V32_CKPT ==="
    python main.py \
        -c configs/finetune_v32_dual_si_scratch.yaml \
        --ckpt "$V32_CKPT" \
        -t False \
...
###############################################################################
# 4. Mod-SpaMo v33 quad-xattn SI scratch
###############################################################################
echo "=== [4a] v33 quad-xattn SI scratch train: $(date) ==="
python main.py \
...
if [ -n "$V33_CKPT" ]; then
    echo "=== [4b] v33 test eval: $V33_CKPT ==="
    python main.py \
        -c configs/finetune_v33_quad_xattn_si_scratch.yaml \
        --ckpt "$V33_CKPT" \
        -t False \
