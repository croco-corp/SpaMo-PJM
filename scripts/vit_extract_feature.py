    ...
    def iterate():
        for i in range(num):
            ...
            if ds_name == "Phoenix14T" or ds_name == "CSL-Daily":
                ...
                if len(videos) > 0:  # type: ignore
                    video_feats = []
                    for j in range(0, len(videos), batch_size):  # type: ignore
                        video_batch = videos[j : min(j + batch_size, len(videos))]  # type: ignore
                        feats = reader.get_feats(video_batch).cpu().numpy()
                        video_feats.append(feats)
                    yield (
                        np.concatenate(video_feats, axis=0),
                        data[i]["fileid"],
                        str(start_time), # type: ignore
                    )  
                else:
                    yield [], data[i]["fileid"], str(start_time)  # type: ignore
...
def main():
    ...
    for m in mode:
        ...
        generator, num = get_iterator(args, _m)  # type: ignore
        iterator = generator()
