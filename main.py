...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-s", "--seed", type=int, default=0, help="Seed for random number generators"
    )
    parser.add_argument(
        "-f",
        "--fast_dev_run",
    ...
    parser.add_argument(
        "--postfix", type=str, default="", help="Additional postfix for log directory"
    )
    parser.add_argument(
        "-l", "--logdir", type=str, default="logs", help="Base directory for logging"
    )
    parser.add_argument(
        "-r", "--resume", default=None, help="Resume training from checkpoint directory"
    )
    parser.add_argument(
        "--no_test", type=bool, default=True, help="Skip test phase after training"
    )
    parser.add_argument(
        "--ckpt", type=str, default=None, help="Checkpoint file for resuming or testing"
    )
    parser.add_argument(
...
def load_configs(config_paths: list[str]) -> OmegaConf:
    ...
    return OmegaConf.merge(*configs)  # pyright: ignore[reportReturnType]
...
def configure_logger(logger_type: str, logdir: str, nowname: str) -> dict:
    ...
    logger_configs = {
        ...
        "tensorboard": {
            ...
            "params": {"version": nowname, "save_dir": logdir},
        },
    }
...
def main():
    ...
    lightning_config = config.pop("lightning", OmegaConf.create())  # pyright: ignore[reportAttributeAccessIssue]
    ...
    data = instantiate_from_config(config.data)  # pyright: ignore[reportAttributeAccessIssue]
    data.setup()

    # Instantiate model
    model = instantiate_from_config(config.model) # pyright: ignore[reportAttributeAccessIssue]
