...
class FlanT5SLT(AbstractSLT):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prepare_models(model_name)  # pyright: ignore[reportArgumentType]
    ...
    def _apply_lora(self) -> None:
        ...
        # Cast to the concrete T5 model type so the static type checker accepts it.
        self.t5_model = get_peft_model(
            cast(T5ForConditionalGeneration, self.t5_model), lora_config
        )  # type: ignore[arg-type]
        print("LoRA adapter applied to T5 model.")

    def _freeze_model(self) -> None:
        """Freeze the T5 model parameters."""
        self.t5_model.eval()  # type: ignore
        for params in self.t5_model.parameters():  # type: ignore
    ...
    def prepare_models(self, t5_model: str) -> None:  # type: ignore
        """
    ...
    def prepare_inputs(
        ...
        input_embeds = self.t5_model.encoder.embed_tokens(input_tokens.input_ids)  # type: ignore
        ...
        joint_mask = create_mask(seq_lengths=new_lengths.tolist(), device=self.device) # type: ignore
    ...
    def prepare_visual_inputs(self, samples: Dict) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if spatial:
            ...
            spatial_mask = create_mask(
                seq_lengths=samples["num_frames"], device=self.device # type: ignore
            )
        ...
        if spatiotemporal:
            ...
            spatiotemporal_mask = create_mask(
                seq_lengths=samples["glor_lengths"], device=self.device # type: ignore
            )

        # Combine features for joint mode
        if self.fusion_mode == "joint":
            bs = spatial_outputs.shape[0] # type: ignore
            spatial_length = spatial_mask.sum(1) # type: ignore
            spatiotemporal_length = spatiotemporal_mask.sum(1) # type: ignore
            new_length = spatial_length + spatiotemporal_length 

            # Concatenate spatial and spatiotemporal features for each sample
            joint_outputs = []
            for i in range(bs):
                valid_spatial_output = spatial_outputs[i, : spatial_length[i], :] # type: ignore
                valid_spatiotemporal_output = spatiotemporal_outputs[ # type: ignore
                    i, : spatiotemporal_length[i], :
                ]
                concat_sample = torch.cat(
                    (valid_spatial_output, valid_spatiotemporal_output), dim=0
                )
                joint_outputs.append(concat_sample)
            joint_outputs = pad_sequence(joint_outputs, batch_first=True)
            ...
            visual_conv_outputs = self.temporal_encoder(
                ...
                torch.tensor(new_length.tolist(), device=self.device), # type: ignore
            )
            ...
            if spatial:
                spatial_conv_outputs = self.temporal_encoder(
                    spatial_outputs.permute(0, 2, 1), # type: ignore
                    torch.tensor(samples["num_frames"], device=self.device), # type: ignore
                )
                ...
                visual_outputs = spatiotemporal_outputs # type: ignore
                visual_masks = spatiotemporal_mask # type: ignore
            else:
    ...
    def visual_textual_align(
        ...
        # Get text embeddings
        text_embeds = self.t5_model.encoder.embed_tokens(output_tokens.input_ids) # type: ignore
    ...
    def shared_step(
        ...
        if split != "train":
            ...
            generated = self.t5_model.generate( # type: ignore
                inputs_embeds=input_embeds,
                attention_mask=input_masks,
                num_beams=5,
                max_length=self.max_txt_len,
    ...
    def configure_optimizers(self):
        ...
        if hasattr(self.trainer, "estimated_stepping_batches"):
            ...
            if hasattr(train_dataloader, "dataloader"):
                train_dataloader = train_dataloader.dataloader # type: ignore

            batches_per_epoch = len(train_dataloader) # type: ignore
            total_steps = batches_per_epoch * max_epochs # type: ignore
        ...
        scheduler = get_cosine_schedule_with_warmup(
            ...
            num_training_steps=total_steps, # type: ignore
        )
